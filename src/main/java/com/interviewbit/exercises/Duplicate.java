...
class Duplicate{
    static int dup(final List<Integer> a){
        int N = a.size();
        ...
        int buckets_len = (int)Math.ceil(sqrt_n);
        int[] memo = new int[buckets_len];
        
        for(int i = 0; i < N; i++){
            int index = (int) ((a.get(i) -1) / sqrt_n);
            memo[index] = memo[index] + 1;
        }

        int overloaded_bucket = -1;
        for(int i = 0; i < buckets_len; i++){
            if(memo[i] > buckets_len){
                overloaded_bucket = i;
            }
        }
        ...
        for(int i = 0; i < N; i++){
            int val = a.get(i);
            if((overloaded_bucket*sqrt_n) <= val &&(val <= (overloaded_bucket + 1)* sqrt_n)){
                if(m.get(val)!= null){
                    return val;
                }
                else{
